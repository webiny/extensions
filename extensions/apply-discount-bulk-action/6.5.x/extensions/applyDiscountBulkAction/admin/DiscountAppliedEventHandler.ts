 ...
 *
 * Registered via `createFeature` in this extension's `admin/Extension.tsx`. The websockets
 * runner resolves every registered `WebsocketEventHandler` and calls `handle` for each
 * incoming message, so we filter by `action` here.
 */
